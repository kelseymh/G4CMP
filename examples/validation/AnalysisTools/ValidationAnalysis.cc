...
void ValidationAnalysis_BoundaryTransmission()
{
  ...
  std::vector<Event> eventVect = ReadInG4CMPStepTextFile("Validation_BoundaryTransmission.txt");
  std::cout << "Done filling events." << std::endl;

  //Primary goal of this analysis function: compute transmission at the Si-Ge interface
  TFile * outFile = new TFile("Validation_BoundaryTransmission.root","RECREATE");
  ...
  TH2F * h_postStepPointYZ = new TH2F("h_postStepPointYZ","h_postStepPointYZ; Step Y [mm]; Step Z [mm]",
				      1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
				      1000,minZ_mm,maxZ_mm);				      				      
  TH2F * h_reflectedFromAboveNextStepEndingsXY = new TH2F("h_reflectedFromAboveNextStepEndingsXY",
							  "h_reflectedFromAboveNextStepEndingsXY; Next step ending X [mm]; Next step ending Y [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1);
  TH2F * h_reflectedFromAboveNextStepEndingsXZ = new TH2F("h_reflectedFromAboveNextStepEndingsXZ",
							  "h_reflectedFromAboveNextStepEndingsXZ; Next step ending X [mm]; Next step ending Z [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,minZ_mm,maxZ_mm);
  TH2F * h_reflectedFromAboveNextStepEndingsYZ = new TH2F("h_reflectedFromAboveNextStepEndingsYZ",
							  "h_reflectedFromAboveNextStepEndingsYZ; Next step ending Y [mm]; Next step ending Z [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,minZ_mm,maxZ_mm);
  TH2F * h_transmittedFromAboveNextStepEndingsXY = new TH2F("h_transmittedFromAboveNextStepEndingsXY",
							    "h_transmittedFromAboveNextStepEndingsXY; Next step ending X [mm]; Next step ending Y [mm]",
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1);
  TH2F * h_transmittedFromAboveNextStepEndingsXZ = new TH2F("h_transmittedFromAboveNextStepEndingsXZ",
							    "h_transmittedFromAboveNextStepEndingsXZ; Next step ending X [mm]; Next step ending Z [mm]",
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							    1000,minZ_mm,maxZ_mm);
  TH2F * h_transmittedFromAboveNextStepEndingsYZ = new TH2F("h_transmittedFromAboveNextStepEndingsYZ",
							    "h_transmittedFromAboveNextStepEndingsYZ; Next step ending Y [mm]; Next step ending Z [mm]",
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							    1000,minZ_mm,maxZ_mm);
  TH2F * h_reflectedFromBelowNextStepEndingsXY = new TH2F("h_reflectedFromBelowNextStepEndingsXY",
							  "h_reflectedFromBelowNextStepEndingsXY; Next step ending X [mm]; Next step ending Y [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1);
  TH2F * h_reflectedFromBelowNextStepEndingsXZ = new TH2F("h_reflectedFromBelowNextStepEndingsXZ",
							  "h_reflectedFromBelowNextStepEndingsXZ; Next step ending X [mm]; Next step ending Z [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,minZ_mm,maxZ_mm);
  TH2F * h_reflectedFromBelowNextStepEndingsYZ = new TH2F("h_reflectedFromBelowNextStepEndingsYZ",
							  "h_reflectedFromBelowNextStepEndingsYZ; Next step ending Y [mm]; Next step ending Z [mm]",
							  1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							  1000,minZ_mm,maxZ_mm);
  TH2F * h_transmittedFromBelowNextStepEndingsXY = new TH2F("h_transmittedFromBelowNextStepEndingsXY",
							    "h_transmittedFromBelowNextStepEndingsXY; Next step ending X [mm]; Next step ending Y [mm]",
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1);
  TH2F * h_transmittedFromBelowNextStepEndingsXZ = new TH2F("h_transmittedFromBelowNextStepEndingsXZ",
							    "h_transmittedFromBelowNextStepEndingsXZ; Next step ending X [mm]; Next step ending Z [mm]",
							    1000,-diskRadius_mm*1.1,diskRadius_mm*1.1,
							    1000,minZ_mm,maxZ_mm);
  TH2F * h_transmittedFromBelowNextStepEndingsYZ = new TH2F("h_transmittedFromBelowNextStepEndingsYZ",
  ...
  for( int iE = 0; iE < eventVect.size(); ++iE ){
	...
	double stepLength3_mm = TMath::Power(dX_3*dX_3 + dY_3*dY_3 + dZ_3*dZ_3,0.5 );
	double dz_3 = z1_3-z0_3;
	...
	//1. If the first step is long and ends within 1 nm of the Si-Ge interface and not on the walls, fill a histo with an entry (i.e. all "incidents")
	//2. If the next step is short and the next-next step goes in the opposite z-direction to the first, then this is a reflection. Fill the same histo
	//   with a different entry representing reflection.
	//3. If the next step is long and the next-next step goes in the same z-direction to the first, then this is a transmission. Fill the same histo
	//   with a different entry representing transmission.
	if( stepLength1_mm > 0.001 ){
	  ...
	  if( r1 < (0.99*diskRadius_mm) && fabs(z1-z_SiGeInterface_mm) < 1e-9 ){
	    ...
	    //Looking at next-step and next-next step. If directions are not the same, then it's a reflection
...
{
  //Read in the events from the textfile
  std::vector<Event> eventVect = ReadInG4CMPStepTextFile("Validation_SCPairbreaking.txt");
  std::cout << "Done filling events." << std::endl;

  //Primary goal of this analysis function: compute transmission at the Si-Ge interface
  TFile * outFile = new TFile("Validation_SCPairbreaking.root","RECREATE");
  
  //Histogram definitions. Note the boilerplate postStepPoint plots are only for phonons
  ...
  TH2F * h_postStepPointYZ = new TH2F("h_postStepPointYZ","h_postStepPointYZ; Step Y [mm]; Step Z [mm]",
				      1000,-barLength_mm*1.1,barLength_mm*1.1,
				      1000,minZ_mm,maxZ_mm);
  TH2F * h_stepDurationVsPhononEnergy = new TH2F("h_stepDurationVsPhononEnergy",
						 "h_stepDurationVsPhononEnergy; Pre-Step Energy [meV]; Step Duration [ns]",
						 nStepsE,minE_meV,maxE_meV,
						 1000,0,1.0);

  TH2F * h_qpEnergyVsPreStepEnergy = new TH2F("h_qpEnergyVsPreStepEnergy",
  ...
  for( int iE = 0; iE < eventVect.size(); ++iE ){
	...
	if( particleName.find("QP") != std::string::npos ){
	  ...
	  if( iS == 0 ){
	    ...
	    //Check to make sure that this QP's event is the same as the phonon we just extracted an energy from	  
	    if( theStep1.eventID == thisPhononEvent ){
	      
	      //Fill cross-check histograms. Have to use kinetic energy because the QPs are given a mass energy
	      //that is artificial so that they get a velocity that is acceptable for diffusion physics.
	      h_qpEnergyVsPreStepEnergy->Fill(thisPhononEnergy_eV*1000.,theStep1.preStepKinEnergy_eV*1000.);
	    }
	  }
	}
      }
  ...
  //Now do fitting of each of the slices of the time histogram and make a graph out of it
  //Setup for fitter and other misc things
  ...
  for( int iBX = 1; iBX < h_stepDurationVsPhononEnergy->GetNbinsX(); ++iBX){
    ...
    if( h_stepDurationVsPhononEnergy->GetXaxis()->GetBinCenter(iBX) < 2*gap_meV ){
    ...
    TH1F * theSlice = (TH1F*)h_stepDurationVsPhononEnergy->ProjectionY(name,iBX,iBX,"");
    ...
    if( iBX < ((int)h_stepDurationVsPhononEnergy->GetNbinsX() / 2.0 ) ){
      ...
      expFit->SetParameter(1,0.19); //Hardcoded start point for low-energy
    }
    else if (iBX == 20) {
      expFit->SetParameter(0,200); //Hardcoded start point (based on number of entries)
...
{
  //Read in the events from the textfile
  std::vector<Event> eventVect = ReadInG4CMPStepTextFile("Validation_QPPhononRadiation.txt");
  std::cout << "Done filling events." << std::endl;
  ...
  TH2F * h_postStepPointXY_qpRad = new TH2F("h_postStepPointXY_qpRad","h_postStepPointXY, Radiation Steps; Step X [mm]; Step Y [mm]",
					    1000,-barWidth_mm*1.1,barWidth_mm*1.1,
					    1000,-barLength_mm*1.1,barLength_mm*1.1);
  TH2F * h_postStepPointXZ_qpRad = new TH2F("h_postStepPointXZ_qpRad","h_postStepPointXZ, Radiation Steps; Step X [mm]; Step Z [mm]",
					    1000,-barWidth_mm*1.1,barWidth_mm*1.1,
					    1000,minZ_mm,maxZ_mm);
  TH2F * h_postStepPointYZ_qpRad = new TH2F("h_postStepPointYZ_qpRad","h_postStepPointYZ, Radiation Steps; Step Y [mm]; Step Z [mm]",
					    1000,-barLength_mm*1.1,barLength_mm*1.1,
					    1000,minZ_mm,maxZ_mm);


  TH2F * h_radiatedEnergyVsQPPreStepEnergy = new TH2F("h_radiatedEnergyVsQPPreStepEnergy",
						      "h_radiatedEnergyVsQPPreStepEnergy; QP Pre-Step Energy [meV]; Energy Radiated [meV];",
						      10*nStepsE,minE_meV,maxE_meV,
						      10*nStepsE,minE_meV,maxE_meV);

  TH2F * h_remainingEnergyVsQPPreStepEnergy = new TH2F("h_remainingEnergyVsQPPreStepEnergy",
						       "h_remainingEnergyVsQPPreStepEnergy; QP Pre-Step Energy [meV]; Energy Radiated [meV];",
						       10*nStepsE,minE_meV,maxE_meV,
						       10*nStepsE,minE_meV,maxE_meV);
  
  TH2F * h_stepDurationVsQPPreStepEnergy = new TH2F("h_stepDurationVsQPPreStepEnergy",
  ...
  for( int iE = 0; iE < eventVect.size(); ++iE ){
	...
	if( particleName.find("QP") != std::string::npos ){
	  ...
	  if( processName.find("qpRadiatesPhonon") != std::string::npos ){
	    ...
	    //For this track, compute the difference in time between the last radiation step and the current radiation step,
	    ...
	    //Plot the deltaT since the last radiation step against the QP pre-energy
	    h_stepDurationVsQPPreStepEnergy->Fill(preStepKinEnergy_eV*1000,thisDeltaT_ns);

	    
	    //Plot the QP's deltaE (i.e. the phonon's radiated energy) vs. QP pre-energy
	    double deltaE_eV = preStepKinEnergy_eV - postStepKinEnergy_eV;
	    h_radiatedEnergyVsQPPreStepEnergy->Fill(preStepKinEnergy_eV*1000,deltaE_eV*1000);
	    h_remainingEnergyVsQPPreStepEnergy->Fill(preStepKinEnergy_eV*1000,postStepKinEnergy_eV*1000);
	  }
	}
      }
    }
  ...
  for( int iBX = 1; iBX < h_stepDurationVsQPPreStepEnergy->GetNbinsX(); ++iBX){
    std::cout << " pre-step energy bin center: " << h_stepDurationVsQPPreStepEnergy->GetXaxis()->GetBinCenter(iBX) << std::endl;
    if( h_stepDurationVsQPPreStepEnergy->GetXaxis()->GetBinCenter(iBX) < 1*gap_meV ){
      std::cout << "skipping this round." << std::endl;
      offset += 1;
      continue;
    }
    ...
    char name[400];
    sprintf(name,"theSlice_%d",iBX);
    TH1F * theSlice = (TH1F*)h_stepDurationVsQPPreStepEnergy->ProjectionY(name,iBX,iBX,"");
    //if( h_stepDurationVsQPPreStepEnergy->GetBinCenter(iBX) < 4*gap_meV ){ theSlice->Rebin(5); }
    TCanvas * c1 = new TCanvas();
    theSlice->Draw("HIST");
    //expFit->SetParameter(0,2000./(theSlice->GetMean()*60)); //Hardcoded start point for low energy 
...
{
  //Read in the events from the textfile
  std::vector<Event> eventVect = ReadInG4CMPStepTextFile("Validation_QPRecombination.txt");
  std::cout << "Done filling events." << std::endl;
  ...
  TH2F * h_postStepPointXY_qpRecomb = new TH2F("h_postStepPointXY_qpRecomb","h_postStepPointXY, Recombination Steps; Step X [mm]; Step Y [mm]",
					    1000,-barWidth_mm*1.1,barWidth_mm*1.1,
					    1000,-barLength_mm*1.1,barLength_mm*1.1);
  TH2F * h_postStepPointXZ_qpRecomb = new TH2F("h_postStepPointXZ_qpRecomb","h_postStepPointXZ, Recombination Steps; Step X [mm]; Step Z [mm]",
					    1000,-barWidth_mm*1.1,barWidth_mm*1.1,
					    1000,minZ_mm,maxZ_mm);
  TH2F * h_postStepPointYZ_qpRecomb = new TH2F("h_postStepPointYZ_qpRecomb","h_postStepPointYZ, Recombination Steps; Step Y [mm]; Step Z [mm]",
					    1000,-barLength_mm*1.1,barLength_mm*1.1,
					    1000,minZ_mm,maxZ_mm);
  ...
  TH2F * h_phononEnergyVsQPEnergy = new TH2F("h_phononEnergyVsQPEnergy",
  ...
  for( int iBX = 1; iBX < h_stepDurationVsQPPreStepEnergy->GetNbinsX(); ++iBX){
    ...
    //Want to put expfit here because that way we can adaptively select the slice mean
    TF1 * expFit = new TF1("expFit","[0]*TMath::Exp(-1*x/[1])",fitMin_ns,2*theSlice->GetMean());
