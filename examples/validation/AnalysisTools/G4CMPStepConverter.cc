...
// This is bidirectional (assumes QP-killing BC on both ends)
TGraph * CalculateTOFPOnTheInterval()
{
  ...
  for( int iP = 0; iP < nPts; ++iP ){
    ...
    //Match limits
    double t_scaling_ns = 6.0*6.0/18.0*1e9; //Scaling to get us to real time (instead of dimensionless time). 6 cm and 18 cm2/s (highly geometry and energy dependent!)
    if(t < 0.033){      
...
//This is an analysis of converted steps to make sure we understand that phonon radiation by
//quasiparticles is working as expected
void AnalyzeConvertedSteps_QPDiffusionProcessStudy(std::string stepFileName)
{

  //Currently what's expected for Geometry 8
  ...
  //Output file
  TFile * outFile = new TFile("SteppingStudies_QPDiffusionProcessStudy.root","RECREATE");  
  ...
  int nStepsNSteps = 2000;
  double minNSteps = 0;
  double maxNSteps = 100000;
  ...
  TH1F * h_QP_nSteps = new TH1F("h_QP_nSteps","QP NSteps; NSteps; QPs",nStepsNSteps,minNSteps,maxNSteps);
  TH1F * h_timeOfLastStepLong = new TH1F("h_timeOfLastStepLong","QP Time of Last Step, Long; Time [ns]; QPs",nStepsTLong,minT_ns,maxTLong_ns*2);
  TH1F * h_timeOfLastStep_RightEdgeLong = new TH1F("h_timeOfLastStep_RightEdgeLong","QP Time of Last Step, Long, for Steps Hitting Right Edge; Time [ns]; QPs",nStepsTLong,minT_ns,maxTLong_ns*2);
  TH1F * h_timeOfLastStep_EitherEdgeLong = new TH1F("h_timeOfLastStep_EitherEdgeLong","QP Time of Last Step, Long, for Steps Hitting Either Edge; Time [ns]; QPs",nStepsTLong,minT_ns,maxTLong_ns*2);
  TH1F * h_timeOfLastStep = new TH1F("h_timeOfLastStep","QP Time of Last Step; Time [ns]; QPs",nStepsT,minT_ns,maxT_ns);
  TH1F * h_timeOfLastStep_RightEdge = new TH1F("h_timeOfLastStep_RightEdge","QP Time of Last Step for Steps Hitting Right Edge; Time [ns]; QPs",nStepsT,minT_ns,maxT_ns);
  TH1F * h_timeOfLastStep_EitherEdge = new TH1F("h_timeOfLastStep_EitherEdge","QP Time of Last Step for Steps Hitting Either Edge; Time [ns]; QPs",nStepsT,minT_ns,maxT_ns);
  TH2F * h_QP_positionOfLastStepXY = new TH2F("h_QP_positionOfLastStepXY","QP Position of Last Step XY; X [mm]; Y [mm];",1000,-40,40,1000,-40,40);
  TH2F * h_QP_positionTimeOfLastStepXT = new TH2F("h_QP_positionTimeOfLastStepXY","QP Position of Last Step XT; X [mm]; T [mm];",1000,-40,40,nStepsT,minT_ns,maxT_ns);
  TH2F * h_QP_positionTimeOfLastStepXTLong = new TH2F("h_QP_positionTimeOfLastStepXYLong","QP Position of Last Step XT; X [mm]; T [mm];",1000,-40,40,nStepsTLong,minT_ns,maxTLong_ns);
  ...
  for( int iSE = 0; iSE < nStepsE; ++iSE ){
    ...
    char nameX[400];
    char nameY[400];
    sprintf(nameR,"h_QP_R_vs_AbsPostStepTime_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameX,"h_QP_X_vs_AbsPostStepTime_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameY,"h_QP_Y_vs_AbsPostStepTime_Energy_%.4f_%.4f",energyLow,energyHigh);    
    TH2F * h_QP_R_vs_AbsPostStepTime = new TH2F(nameR,"QP Radial position vs. absolute post-step time; R [mm]; post-step T [ns]; nQPs",nStepsR,minR_mm,maxR_mm,nStepsT,minT_ns,maxT_ns);
    TH2F * h_QP_X_vs_AbsPostStepTime = new TH2F(nameX,"QP X position vs. absolute post-step time; X [mm]; post-step T [ns]; nQPs",nStepsX,minX_mm,maxX_mm,nStepsT,minT_ns,maxT_ns);
    TH2F * h_QP_Y_vs_AbsPostStepTime = new TH2F(nameY,"QP Y position vs. absolute post-step time; Y [mm]; post-step T [ns]; nQPs",nStepsY,minY_mm,maxY_mm,nStepsT,minT_ns,maxT_ns);
    map_energyBin_QPRVsTime.emplace(energyBin,h_QP_R_vs_AbsPostStepTime);
    map_energyBin_QPXVsTime.emplace(energyBin,h_QP_X_vs_AbsPostStepTime);
    map_energyBin_QPYVsTime.emplace(energyBin,h_QP_Y_vs_AbsPostStepTime);


    

    sprintf(nameR,"h_QP_R_vs_AbsPostStepTimeLong_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameX,"h_QP_X_vs_AbsPostStepTimeLong_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameY,"h_QP_Y_vs_AbsPostStepTimeLong_Energy_%.4f_%.4f",energyLow,energyHigh);    
    TH2F * h_QP_R_vs_AbsPostStepTimeLong = new TH2F(nameR,"QP Radial position vs. absolute post-step time; R [mm]; post-step T [ns]; nQPs",nStepsR,minR_mm,maxR_mm,nStepsTLong,minT_ns,maxTLong_ns);
    TH2F * h_QP_X_vs_AbsPostStepTimeLong = new TH2F(nameX,"QP X position vs. absolute post-step time; X [mm]; post-step T [ns]; nQPs",nStepsX,minX_mm,maxX_mm,nStepsTLong,minT_ns,maxTLong_ns);
    TH2F * h_QP_Y_vs_AbsPostStepTimeLong = new TH2F(nameY,"QP Y position vs. absolute post-step time; Y [mm]; post-step T [ns]; nQPs",nStepsY,minY_mm,maxY_mm,nStepsTLong,minT_ns,maxTLong_ns);    
    map_energyBin_QPRVsTimeLong.emplace(energyBin,h_QP_R_vs_AbsPostStepTimeLong);
    map_energyBin_QPXVsTimeLong.emplace(energyBin,h_QP_X_vs_AbsPostStepTimeLong);
    map_energyBin_QPYVsTimeLong.emplace(energyBin,h_QP_Y_vs_AbsPostStepTimeLong);
  }
	...
	if( particleName == "QP" ){
	  ...
	  for(std::map<std::pair<double,double>,TH2F*>::iterator it = map_energyBin_QPRVsTime.begin(); it != map_energyBin_QPRVsTime.end(); ++it ){
	    ...
	    if( e0*1000 < maxEnergy && e0*1000 >= minEnergy ){
	      map_energyBin_QPRVsTime[it->first]->Fill(r1,t1);
	      map_energyBin_QPXVsTime[it->first]->Fill(x1,t1);
	      map_energyBin_QPYVsTime[it->first]->Fill(y1,t1);


	      map_energyBin_QPRVsTimeLong[it->first]->Fill(r1,t1);
	      map_energyBin_QPXVsTimeLong[it->first]->Fill(x1,t1);
	      map_energyBin_QPYVsTimeLong[it->first]->Fill(y1,t1);	      
	    }
	  }

	  //Print last step times.
	  if( iS == theTrack.stepVect.size()-1 ){
	    h_timeOfLastStepLong->Fill(t1);
	    h_timeOfLastStep->Fill(t1);
	    h_QP_positionOfLastStepXY->Fill(x1,y1);
	    h_QP_positionTimeOfLastStepXT->Fill(x1,t1);
	    h_QP_positionTimeOfLastStepXTLong->Fill(x1,t1);
	    
	    //For Geometry 5
	    if( x1 > 29.999 ){
	      h_timeOfLastStep_RightEdgeLong->Fill(t1);
	      h_timeOfLastStep_RightEdge->Fill(t1);
	    }
	    //For Geometry 8
	    if( x1 > 29.999 || x1 < -29.999 ){
	      h_timeOfLastStep_EitherEdgeLong->Fill(t1);
	      h_timeOfLastStep_EitherEdge->Fill(t1);
	    }
	  }
	}
      }
      h_QP_nSteps->Fill(nQPSteps);
    }    
  }
  ...
  h_timeOfLastStep_EitherEdgeLong->Draw("HIST");
  g_TOFP_expected->SetLineStyle(1);
  g_TOFP_expected->SetLineColor(kRed);
  double binWidth_ns = h_timeOfLastStep_EitherEdgeLong->GetBinWidth(1);
  h_timeOfLastStep_EitherEdgeLong->Scale(1.0/((double)h_timeOfLastStep_EitherEdgeLong->Integral())/binWidth_ns);
  h_timeOfLastStep_EitherEdgeLong->Draw("HIST");
  g_TOFP_expected->Draw("Lsame");
  TLegend * l2 = new TLegend();
  l2->AddEntry(h_timeOfLastStep_EitherEdgeLong,"TOFP, Either Nb End");
  l2->AddEntry(g_TOFP_expected,"TOFP, Theoretically Expected for Sim. Geometry 8");
  l2->Draw("same");
...
void AnalyzeConvertedSteps_BogoliubovQPDiffusionProcessStudy_BoundaryLimitedDiffusion(std::string stepFileName)
{
  ...
  for( int iSE = 0; iSE < nStepsE; ++iSE ){
    ...
    sprintf(nameX,"h_QP_X_vs_AbsPostStepTime_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameY,"h_QP_Y_vs_AbsPostStepTime_Energy_%.4f_%.4f",energyLow,energyHigh);    
    TH2F * h_QP_X_vs_AbsPostStepTime = new TH2F(nameX,"QP X position vs. absolute pre-step time; X [mm]; Pre-step T [ns]; nQPs",nStepsX,minX_mm,maxX_mm,nStepsT,minT_ns,maxT_ns);
    TH2F * h_QP_Y_vs_AbsPostStepTime = new TH2F(nameY,"QP Y position vs. absolute pre-step time; Y [mm]; Pre-step T [ns]; nQPs",nStepsY,minY_mm,maxY_mm,nStepsT,minT_ns,maxT_ns);
    map_energyBin_QPXVsTime.emplace(energyBin,h_QP_X_vs_AbsPostStepTime);
    map_energyBin_QPYVsTime.emplace(energyBin,h_QP_Y_vs_AbsPostStepTime);
    ...
    sprintf(nameX,"h_QP_X_vs_AbsPostStepTimeLong_Energy_%.4f_%.4f",energyLow,energyHigh);
    sprintf(nameY,"h_QP_Y_vs_AbsPostStepTimeLong_Energy_%.4f_%.4f",energyLow,energyHigh);    
    TH2F * h_QP_X_vs_AbsPostStepTimeLong = new TH2F(nameX,"QP X position vs. absolute pre-step time; X [mm]; Pre-step T [ns]; nQPs",nStepsX,minX_mm,maxX_mm,nStepsTLong,minT_ns,maxTLong_ns);
    TH2F * h_QP_Y_vs_AbsPostStepTimeLong = new TH2F(nameY,"QP Y position vs. absolute pre-step time; Y [mm]; Pre-step T [ns]; nQPs",nStepsY,minY_mm,maxY_mm,nStepsTLong,minT_ns,maxTLong_ns);
    map_energyBin_QPXVsTimeLong.emplace(energyBin,h_QP_X_vs_AbsPostStepTimeLong);
    map_energyBin_QPYVsTimeLong.emplace(energyBin,h_QP_Y_vs_AbsPostStepTimeLong);
  }
