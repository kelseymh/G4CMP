...
G4CMPLukeScattering::G4CMPLukeScattering()
  ...
  std::ifstream LUT("luke_mfp.csv");

  if (LUT.good()) {
    LUT >> ESIZE >> EMAX >> THETASIZE >> THETAMAX >> KSIZE >> KMAX;
  }
  MeanFreeTimes = std::vector<std::vector<std::vector<G4double> > >(ESIZE, std::vector<std::vector<G4double> >(THETASIZE, std::vector<G4double>(KSIZE, 0.)));
    for (G4int i=0; i<ESIZE; ++i) {
      for (G4int j=0; j<THETASIZE; ++j) {
        for (G4int k=0; k<KSIZE; ++k) {
          LUT >> MeanFreeTimes[i][j][k];
        }
      }
    }
...
G4CMPLukeScattering::GetMeanFreePath(const G4Track& aTrack,
  ...
  *condition = NotForced;
  ...
  if (verboseLevel > 1) {
    G4cout << "LukeScattering kmag = " << k0.mag()*m << G4endl;
  }

  G4int kbin = round(k0.mag()/kSound/(KMAX/KSIZE)); // .1 resolution in k map
  ...
  if (fMan && fMan->DoesFieldExist()) {
    ...
    thetabin = round(acos((k0*E0)/k0.mag()/E0.mag())/(THETAMAX/THETASIZE));// 29 bins
  }

  G4int ebin = round(E0.mag()/(EMAX/ESIZE));
  G4double mfp = MeanFreeTimes[ebin][0][kbin]*um;
  if (verboseLevel > 1) {
...
G4double G4CMPLukeScattering::ComputeTimeSteps(const G4Track& aTrack) {
  G4CMPTrackInformation* trackInfo = static_cast<G4CMPTrackInformation*>(
    aTrack.GetAuxiliaryTrackInformation(fPhysicsModelID));
...
G4double G4CMPLukeScattering::TimeStepInField(G4double Emag, G4double coeff, G4double l0) const {
...
G4double G4CMPLukeScattering::CalculateEmissionRate(
                                        const G4CMPTrackInformation* trackInfo, 
                                        const G4ThreeVector& k, 
                                        const G4ThreeVector& E, 
                                        G4double charge, 
                                        G4double t) const {
  return std::sqrt(k*k + charge*charge/hbar_Planck/hbar_Planck*(k*E)*t*t +
                   charge/hbar_Planck*(E*E)*t);
}
