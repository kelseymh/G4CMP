...
G4ThreeVector
G4CMPDriftBoundaryProcess::LambertianReflection(const G4LatticePhysical* lattice,
			   const G4ThreeVector& surfNorm, G4int valley) const {
  const G4ThreeVector surfPoint = GetCurrentTrack()->GetPosition();
  return LambertianReflection(lattice, surfNorm, valley, surfPoint);
}
...
G4CMPDriftBoundaryProcess::LambertianReflection(const G4LatticePhysical* lattice,
  ...
  do {
    reflectedDir = GetLambertianVector(surfNorm);
           ...
           !ChargeVelocityIsInward(lattice, valley, reflectedDir, surfNorm,
                                   surfPoint));
...
DoDiffuseElectron(const G4ThreeVector& surfNorm,
  ...
  G4ThreeVector p = G4CMPBoundaryUtils::GetLambertianVector(surfNorm);
  return p;
}
