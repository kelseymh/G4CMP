...
// 20269121  G4CMP-501 -- Add specular and diffuse transmission of phonons
...
G4bool G4CMPBoundaryUtils::IsGoodBoundary(const G4Step& aStep) {
  ...
  return (IsBounaryStep(aStep) && GetBoundingVolumes(aStep) &&
...
void G4CMPBoundaryUtils::ApplyBoundaryAction(
  ...
  if (!latNear || !latFar) {
    if (!matTable) {
      ...
      if (buVerboseLevel > 5) {
        G4cout
            << "ABA Function Point A | Doing Absorption in ApplyBoundaryAction"
            << G4endl;
      }
      DoAbsorption(aTrack, aStep, aParticleChange);
    } else if (MaximumReflections(aTrack)) {
      if (buVerboseLevel > 5) {
        G4cout << "ABA Function Point B | Maximum Reflections reached in "
                  "ApplyBoundaryAction"
               << G4endl;
      }
      DoSimpleKill(aTrack, aStep, aParticleChange);
      ...
      if (buVerboseLevel > 5) {
        G4cout
            << "ABA Function Point C | Doing Reflection in ApplyBoundaryAction"
            << G4endl;
      }
      ...
      if (buVerboseLevel > 5) {
        G4cout
            << "ABA Function Point D | Doing Simple Kill in ApplyBoundaryAction"
            << G4endl;
      }
      DoSimpleKill(aTrack, aStep, aParticleChange);
    }
    ...
    if (!matTable) {
      DoSimpleKill(aTrack, aStep, aParticleChange);
    } else if (G4UniformRand() < GetMaterialProperty("specProb") &&
               DoLatticeInterface(aTrack, aStep, aParticleChange)) {
      if (buVerboseLevel > 5) {
        G4cout << "ABA Function Point E | Doing anisotropic lattice-interface "
                  "interaction"
               << G4endl;
      }
    } else if (G4UniformRand() < GetMaterialProperty("reflProb")) {
      if (MaximumReflections(aTrack)) {
        if (buVerboseLevel > 5) {
          G4cout << "ABA Function Point F | Maximum Reflections reached in "
        ...
        if (buVerboseLevel > 5) {
          G4cout << "ABA Function Point G | Doing diffuse reflection" << G4endl;
        }
        IncrementReflectionCount(aTrack);
        DoDiffuseReflection(aTrack, aStep, aParticleChange);
      }
    } else {
      if (buVerboseLevel > 5) {
        G4cout << "ABA Function Point H | Doing diffuse transmission" << G4endl;
      }
      DoDiffuseTransmission(aTrack, aStep, aParticleChange);
    }
  }
}
