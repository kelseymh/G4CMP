...
// If we arrive in a new lattice, either compute the pair breaking rate and
// then update the lookup table, or update the lookup table using a computed
// rate curve that already exists in the map
void
G4CMPSCPairBreakingRate::UpdateLookupTable(const G4LatticePhysical * theLat) {
...
G4CMPSCPairBreakingRate::ComputeNormalizedTauPairBreakingVsEnergy() {
  ...
  G4double deltaPhononEnergyDivGap =
    (fMaxPhononEnergyDivGap - fMinPhononEnergyDivGap) /
    ((double)fPhononEnergyBins);  
  ...
  for (int iB = 0; iB < fPhononEnergyBins; ++iB) {
    ...
    G4double phononEnergy = fGapEnergy *
      (deltaPhononEnergyDivGap * (iB+0.5) + fMinPhononEnergyDivGap);

    //--------------------------------------------------
    //Now do the calculation for this phonon energy       
    //Initially, going to try this without specifying units -- shouuuuuld be
    //fine but if there are problems this is the first place to debug.
    //Note that in what follows, omega indicates the QP energy integrated over.

    //Missing a factor of Alpha2 because it cancels later
    double prefactor = 4 * pi / hbar_Planck ;
    int nW = 20000;
    ...
    for (int iW = 0; iW < nW; ++iW) {
      ...
      //So that omega lands in the middle of a bin
    ...
    //Missing a factor of 1/Alpha2 because it cancels later
    double tau0 = hbar_Planck / 4 / pi / pi / fGap0Energy; 
