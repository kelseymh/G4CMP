...
// 20250204  G4CMP-459 -- Support reflection displacement search at hard
// corners. 20250325  G4CMP-463 -- Set surface step size & limit with macro
// command. 20250402  G4CMP-468 -- Support position change after surface
// displacement. 20250413  M. Kelsey -- Protect debugging output with verbosity.
// 20250422  N. Tenpas -- Add position arguments for PhononVelocityIsInward
// test. 20250423  G4CMP-468 -- Add wrapper function for updating navigator.
...
// 20250505  G4CMP-471 -- Update diagnostic output for surface displacement
// loop. 20250814  G4CMP-496 -- Fix phonon wavevector & momentum direction
// coming out of DoDecay. 20260901  G4CMP-501 -- Add anisotropic refraction and
...
void G4CMPPhononBoundaryProcess::LoadDataForTrack(
    const G4Track* track, const G4bool overrideMomentumReset) {
...
G4double G4CMPPhononBoundaryProcess::PostStepGetPhysicalInteractionLength(
...
G4double G4CMPPhononBoundaryProcess::GetMeanFreePath(
    const G4Track& aTrack, G4double /*prevStepLength*/,
    G4ForceCondition* condition) {
  if (verboseLevel > 5) {
...
G4VParticleChange* G4CMPPhononBoundaryProcess::PostStepDoIt(
  ...
  if (verboseLevel > 1) {
    ...
    G4cout << GetProcessName() << "::PostStepDoIt "
           << "Event " << eID << " Track " << aTrack.GetTrackID() << " Step "
...
G4bool G4CMPPhononBoundaryProcess::DoLatticeInterface(
    const G4Track& aTrack, const G4Step& aStep,
    G4ParticleChange& aParticleChange) {
  // make sure the surface property has a registered table
  // for the transmitted and reflected phonon modes

  if (!surfProp || !surfProp->HasPhononInterfaceTable()) {
    return false;
  }

  const std::shared_ptr<const G4CMPInterfaceTable>& table =
      surfProp->GetPhononInterfaceTable();
  ...
  const G4VPhysicalVolume* preVolume = prePoint->GetPhysicalVolume();

  const G4VPhysicalVolume* postVolume = postPoint->GetPhysicalVolume();

  if (!preVolume || !postVolume) {
    return false;
  }

  // determine the direction that the interface is being crossed

  G4CMPInterfaceTable::Side incidentSide;

  const std::string preName = std::string(preVolume->GetName().c_str());

  const std::string postName = std::string(postVolume->GetName().c_str());

  if (preName == table->GetVolumeA() && postName == table->GetVolumeB()) {
    incidentSide = G4CMPInterfaceTable::Side::A;

  } else if (preName == table->GetVolumeB() &&
             postName == table->GetVolumeA()) {
    incidentSide = G4CMPInterfaceTable::Side::B;

  } else {
    return false;
  }

  const G4VPhysicalVolume* volumeA =
      incidentSide == G4CMPInterfaceTable::Side::A ? preVolume : postVolume;

  const G4VPhysicalVolume* volumeB =
      incidentSide == G4CMPInterfaceTable::Side::A ? postVolume : preVolume;

  const G4VTouchable* touchableA = incidentSide == G4CMPInterfaceTable::Side::A
                                       ? prePoint->GetTouchable()
                                       : postPoint->GetTouchable();

  const G4VTouchable* touchableB = incidentSide == G4CMPInterfaceTable::Side::A
                                       ? postPoint->GetTouchable()
                                       : prePoint->GetTouchable();
  ...
  const G4int incidentMode =
      G4PhononPolarization::Get(aTrack.GetParticleDefinition());

  G4CMPPhononTrackInfo* incidentInfo =
      G4CMP::GetTrackInfo<G4CMPPhononTrackInfo>(aTrack);

  if (!incidentInfo) {
    if (verboseLevel) {
      G4cerr << GetProcessName() << "::DoLatticeInterface: phonon has no "
             << "G4CMPPhononTrackInfo." << G4endl;
    }
  ...
  // enforce maximum reflections internally

  if (MaximumReflections(aTrack)) {
    if (verboseLevel > 1) {
      G4cout << GetProcessName()
             << "::DoLatticeInterface: maximum reflection count reached ("
             << incidentInfo->ReflectionCount() << "). Killing phonon."
             << G4endl;
    }
  ...
  const G4ThreeVector incidentKGlobal = incidentInfo->k();

  if (incidentKGlobal.mag2() == 0.) {
    if (verboseLevel) {
      G4cerr << GetProcessName()
             << "::DoLatticeInterface: zero incident wavevector." << G4endl;
    }
  ...
  const G4VTouchable* incidentTouchable = prePoint->GetTouchable();

  if (!incidentTouchable) {
    DoSimpleKill(aTrack, aStep, aParticleChange);
    return true;
  }

  // put the incoming wavevector into local coordinates

  G4ThreeVector incidentKSolid = incidentKGlobal.unit();

  G4CMP::RotateToLocalDirection(incidentTouchable, incidentKSolid);

  incidentKSolid = incidentKSolid.unit();

  // convert to the interface frame

  G4ThreeVector incidentKInterface;

  if (surfProp->HasPhononInterfaceFrame()) {
    const G4RotationMatrix& solidToInterface =
        incidentSide == G4CMPInterfaceTable::Side::A
            ? surfProp->GetPhononSolidAToInterface()
            : surfProp->GetPhononSolidBToInterface();

    incidentKInterface = solidToInterface * incidentKSolid;
  } else {
    incidentKInterface = incidentKGlobal.unit();
  }

  incidentKInterface = incidentKInterface.unit();

  // map the incident direction onto the table grid

  G4int matchedTheta = -1;
  G4int matchedPhi = -1;

  const G4CMPInterfaceTable::Entry* entry =
      table->LookupNearest(incidentSide, incidentMode, incidentKInterface,
                           &matchedTheta, &matchedPhi);

  if (!entry || entry->status != G4CMPInterfaceTable::Status::Populated ||
      entry->nOutcomes <= 0) {
    if (verboseLevel) {
      G4cerr << GetProcessName()
             << "::DoLatticeInterface: anisotropic-interface "
             << "table lookup failed for " << preName << " -> " << postName
             << ", mode " << G4PhononPolarization::Label(incidentMode)
             << ", k(global)=" << incidentKGlobal.unit()
             << ", k(solid)=" << incidentKSolid
             << ", k(interface)=" << incidentKInterface << G4endl;
    }
  ...
  // sample one outgoing direction using our previously calculated
  // probabilities

  const G4double target = G4UniformRand() * entry->probabilitySum;

  G4double cumulative = 0.0;

  const G4CMPInterfaceTable::Outcome* selected = nullptr;

  for (G4int i = 0; i < entry->nOutcomes; ++i) {
    const auto& outcome = entry->outcomes[static_cast<std::size_t>(i)];
  ...
  // protect from floating point errors

  if (!selected) {
    selected = &entry->outcomes[static_cast<std::size_t>(entry->nOutcomes - 1)];
  }

  if (selected->mode < 0 || selected->mode >= G4PhononPolarization::NUM_MODES ||
      selected->kDir.mag2() == 0.0 || selected->vgDir.mag2() == 0.0) {
    if (verboseLevel) {
      G4cerr << GetProcessName()
             << "::DoLatticeInterface: invalid outgoing table state." << G4endl;
    }
  ...
  // if we are on the same side as the incidence, then reflection

  const G4bool reflected = selected->outgoingSide == incidentSide;

  const G4VPhysicalVolume* targetVolume =
      selected->outgoingSide == G4CMPInterfaceTable::Side::A ? volumeA
                                                             : volumeB;

  const G4VTouchable* targetTouchable =
      selected->outgoingSide == G4CMPInterfaceTable::Side::A ? touchableA
                                                             : touchableB;

  if (!targetVolume || !targetTouchable) {
    DoSimpleKill(aTrack, aStep, aParticleChange);
    return true;
  }

  G4LatticePhysical* targetLattice =
      G4LatticeManager::GetLatticeManager()->GetLattice(targetVolume);

  if (!targetLattice) {
    if (verboseLevel) {
      G4cerr << GetProcessName() << "::DoLatticeInterface: destination volume "
             << targetVolume->GetName() << " has no lattice." << G4endl;
    }
  ...
  // allow for mode conversion

  G4ParticleDefinition* definition = nullptr;

  if (selected->mode == G4PhononPolarization::Long) {
    definition = G4PhononLong::Definition();

  } else if (selected->mode == G4PhononPolarization::TransSlow) {
    definition = G4PhononTransSlow::Definition();

  } else if (selected->mode == G4PhononPolarization::TransFast) {
    definition = G4PhononTransFast::Definition();
  }
  ...
  G4ThreeVector kSolid;
  G4ThreeVector vgSolid;
  G4ThreeVector kGlobal;
  G4ThreeVector vgGlobal;

  if (surfProp->HasPhononInterfaceFrame()) {
    const G4RotationMatrix& solidToInterface =
        selected->outgoingSide == G4CMPInterfaceTable::Side::A
            ? surfProp->GetPhononSolidAToInterface()
            : surfProp->GetPhononSolidBToInterface();

    const G4RotationMatrix interfaceToSolid = solidToInterface.inverse();

    kSolid = interfaceToSolid * selected->kDir.unit();

    vgSolid = interfaceToSolid * selected->vgDir.unit();

    kSolid = kSolid.unit();

    vgSolid = vgSolid.unit();

    kGlobal = kSolid;

    vgGlobal = vgSolid;

    G4CMP::RotateToGlobalDirection(targetTouchable, kGlobal);

    G4CMP::RotateToGlobalDirection(targetTouchable, vgGlobal);

    kGlobal = kGlobal.unit();

    vgGlobal = vgGlobal.unit();
  } else {
    kGlobal = selected->kDir.unit();

    vgGlobal = selected->vgDir.unit();

    kSolid = kGlobal;

    vgSolid = vgGlobal;

    G4CMP::RotateToLocalDirection(targetTouchable, kSolid);

    G4CMP::RotateToLocalDirection(targetTouchable, vgSolid);
  ...
  const G4double velocity = targetLattice->MapKtoV(selected->mode, kSolid);

  // Convert the interface normal to global coordinates

  G4ThreeVector normalAtoBGlobal;

  if (surfProp->HasPhononInterfaceFrame()) {
    const G4RotationMatrix& solidAToInterface =
        surfProp->GetPhononSolidAToInterface();

    const G4RotationMatrix interfaceToSolidA = solidAToInterface.inverse();

    G4ThreeVector normalASolid =
        interfaceToSolidA * table->GetNormalAtoB().unit();

    normalASolid = normalASolid.unit();

    normalAtoBGlobal = normalASolid;

    G4CMP::RotateToGlobalDirection(touchableA, normalAtoBGlobal);

    normalAtoBGlobal = normalAtoBGlobal.unit();
  } else {
    normalAtoBGlobal = table->GetNormalAtoB().unit();
  }

  // Nudge the phonon across the boundary

  const G4double sideSign =
      selected->outgoingSide == G4CMPInterfaceTable::Side::A ? -1.0 : +1.0;

  const G4double boundaryNudge = 10.0 * nm;

  const G4ThreeVector newPosition =
      postPoint->GetPosition() + sideSign * boundaryNudge * normalAtoBGlobal;

  // kill and recreate the phonon for mode conversion

  G4DynamicParticle* dynamic =
      new G4DynamicParticle(definition, vgGlobal, aTrack.GetKineticEnergy());

  G4Track* secondary =
      new G4Track(dynamic, postPoint->GetGlobalTime(), newPosition);

  secondary->SetParentID(aTrack.GetTrackID());

  secondary->SetWeight(aTrack.GetWeight());

  secondary->SetCreatorProcess(this);

  secondary->SetMomentumDirection(vgGlobal);

  secondary->SetVelocity(velocity);

  secondary->UseGivenVelocity(true);

  G4CMP::AttachTrackInfo(*secondary, kGlobal);

  G4CMPPhononTrackInfo* newInfo =
      G4CMP::GetTrackInfo<G4CMPPhononTrackInfo>(*secondary);

  // make sure we still have the reflection history

  if (newInfo) {
    const std::size_t oldReflectionCount = incidentInfo->ReflectionCount();

    for (std::size_t i = 0; i < oldReflectionCount; ++i) {
      newInfo->IncrementReflectionCount();
    }
  ...
  if (verboseLevel > 2) {
    G4cout
        << "\n"
        << GetProcessName() << "::DoLatticeInterface"
        << "\n  " << preName << " -> " << postName
        << "\n  frame configured = " << surfProp->HasPhononInterfaceFrame()
        << "\n  incident side = "
        << (incidentSide == G4CMPInterfaceTable::Side::A ? "A" : "B")
        << "\n  incident mode = " << G4PhononPolarization::Label(incidentMode)
        << "\n  incident k(global) = " << incidentKGlobal.unit()
        << "\n  incident k(solid) = " << incidentKSolid
        << "\n  incident k(interface) = " << incidentKInterface
        << "\n  matched grid = (" << matchedTheta << "," << matchedPhi << ")"
        << "\n  selected side = "
        << (selected->outgoingSide == G4CMPInterfaceTable::Side::A ? "A" : "B")
        << "\n  selected mode = " << G4PhononPolarization::Label(selected->mode)
        << "\n  probability = " << selected->probability
        << "\n  reflected = " << reflected
        << "\n  outgoing k(interface) = " << selected->kDir.unit()
        << "\n  outgoing k(solid) = " << kSolid
        << "\n  outgoing k(global) = " << kGlobal
        << "\n  outgoing vg(interface) = " << selected->vgDir.unit()
        << "\n  outgoing vg(solid) = " << vgSolid
        << "\n  outgoing vg(global) = " << vgGlobal
        << "\n  normal A->B(global) = " << normalAtoBGlobal
        << "\n  outgoing speed = " << velocity / (m / s) << " m/s" << G4endl;
  }

  // replace the incident track with the final track
...
void G4CMPPhononBoundaryProcess::DoReflection(
    const G4Track& aTrack, const G4Step& aStep,
  ...
  if (random < downconversionProb) {
    ...
    // again
    G4ThreeVector vec1 =
        G4CMP::GetLambertianVector(theLattice, surfNorm, mode1, surfacePoint);
    G4ThreeVector vec2 =
        G4CMP::GetLambertianVector(theLattice, surfNorm, mode2, surfacePoint);
    ...
    reflectedKDir = GetSpecularVector(waveVector, surfNorm, mode, surfacePoint);

    refltype = "specular";
  } else {
    if (verboseLevel > 2) {
      G4cout << "Diffuse reflection at boundary, with surface norm " << surfNorm
             << ", in lattice: " << theLattice
    ...
    if (verboseLevel > 5) {
      G4cout << "DR Function Point H | Diffuse reflection at boundary, with "
             << "surface norm " << surfNorm << ", in lattice: " << theLattice
             << ", at surfacePoint: " << surfacePoint << "." << G4endl;
    }

    reflectedKDir =
        G4CMP::GetLambertianVector(theLattice, surfNorm, mode, surfacePoint);
    refltype = "diffuse";
  }
  ...
  if (reflectedKDir.mag() < 1e-10) {
    G4String msg =
        "ReflectedKDir has zero length, implying that a Lambertian "
  ...
  if (refltype == "specular" &&
      *phParticleChange.GetPosition() != surfacePoint) {
  ...
  if (verboseLevel > 5) {
    G4cout << "DR Function Point I | Phonon incident waveVector: " << waveVector
           << ", non-generalized surfNorm: " << surfNorm
           << ", and attempted reflected k-vector " << reflectedKDir << G4endl;
    G4cout << "Note: after mods, above surfnorm should in fact be generalized."
           << G4endl;
  }

  if (surfNorm.dot(initialVDir.unit()) <= 0.0) {
    surfNorm *= -1;
  }
...
G4ThreeVector G4CMPPhononBoundaryProcess::GetSpecularVector(
    const G4ThreeVector& waveVector, G4ThreeVector& surfNorm, G4int mode,
  ...
  G4ThreeVector kPerpV =
      kPerpMag * newNorm;  // Negative implied in kPerpMag for inward pointing
  G4ThreeVector kTan =
      reflectedKDir - kPerpV;  // Get kTan: reflectedKDir = kPerpV + kTan
  ...
  if (verboseLevel > 3) {
    G4cout << "GetSpecularVector:beforeLoop -> "
           ...
           << ", reflectedKDir = " << reflectedKDir << ", newNorm = " << newNorm
  ...
  // Assumes everything is in Global. Just add the GetGlobal in the loop
  // conditions.
  while (!G4CMP::PhononVelocityIsInward(
             theLattice, mode, GetGlobalDirection(reflectedKDir),
             GetGlobalDirection(newNorm), GetGlobalPosition(stepLocalPos)) &&
  ...
  if (!G4CMP::PhononVelocityIsInward(theLattice, mode, reflectedKDir, newNorm,
    ...
    if (verboseLevel) {
      G4cerr << GetProcessName() << "::GetSpecularVector"
             ...
             << " attempts." << G4endl;
      if (verboseLevel > 1) {
        G4cout << "Doing diffuse reflection at surface point " << surfPoint / mm
               << " mm" << G4endl;
      }
    }
    ...
    newNorm = surfNorm;
    reflectedKDir =
        G4CMP::GetLambertianVector(theLattice, surfNorm, mode, surfPoint);
  }

  if (verboseLevel > 3) {
    G4cout << GetProcessName() << "::GetSpecularVector"
           << ": nAttempts = " << nAttempts << ", waveVector = " << waveVector
...
void G4CMPPhononBoundaryProcess::UpdateNavigatorVolume(
    const G4Step& step, const G4ThreeVector& position,
...
// function
void G4CMPPhononBoundaryProcess::DoTransmission(
    const G4Track& aTrack, const G4Step& aStep,
    G4ParticleChange& /*particleChange*/) {
  // Debugging
  if (verboseLevel > 5) {
    ...
    G4cout << "DT Function Point A | Lattice manager current lattice: "
           << G4LatticeManager::GetLatticeManager()->GetLattice(
  ...
  G4LatticePhysical* latNear =
      G4LatticeManager::GetLatticeManager()->GetLattice(
          aStep.GetPreStepPoint()->GetPhysicalVolume());
  G4LatticePhysical* latFar = G4LatticeManager::GetLatticeManager()->GetLattice(
      aStep.GetPostStepPoint()->GetPhysicalVolume());
  if (!latNear || !latFar) {
  ...
  this->SetLattice(G4LatticeManager::GetLatticeManager()->GetLattice(
      aStep.GetPostStepPoint()->GetPhysicalVolume()));
  anharmonicDecay->SetLattice(G4LatticeManager::GetLatticeManager()->GetLattice(
      aStep.GetPostStepPoint()->GetPhysicalVolume()));
  ...
  G4CMP::RotateToGlobalDirection(aStep.GetPostStepPoint()->GetTouchable(),
                                 waveVector);
  ...
  if (!G4CMP::PhononVelocityIsOutward(theLattice, mode, waveVector,
    ...
    for (int iT = 0; iT < nTries; ++iT) {
      ...
      if (G4CMP::PhononVelocityIsOutward(theLattice, mode, newAttemptWaveVector,
        ...
        // Put the newAttemptWaveVector and the vdir back into a global
        // direction
    ...
    if (!successfulTransmission) {
      G4String msg =
          G4PhononPolarization::Name(mode) +
          " transmission failed even after repeated tries to nudge direction.";

      if (verboseLevel > 5) {
        G4cout << "DT Function Point H | Phonon incident waveVector: "
               << waveVector.unit()
               << ", generalized surfNorm: " << generalizedSurfNorm
               << ", attempted transmitted k-vector " << waveVector.unit()
               << ", and attempted transmitted vDir: " << vdir << G4endl;
      }
      G4Exception((GetProcessName() + "::DoTransmission").c_str(),
                  "Boundary011", JustWarning, msg.c_str());
      DoSimpleKill(aTrack, aStep, phParticleChange);
    }
    return;
  }
...
// basically the same as the function above but with diffuse transmission
// I kept it as a reference for now

void G4CMPPhononBoundaryProcess::DoDiffuseTransmission(
    const G4Track& aTrack, const G4Step& aStep,
    G4ParticleChange& particleChange) {
  const G4StepPoint* prePoint = aStep.GetPreStepPoint();
  ...
  if (!latNear || !latFar) {
    ...
    msg << "Diffuse transmission requires registered lattices "
        << "on both sides of the boundary.";

    G4Exception("G4CMPPhononBoundaryProcess::DoDiffuseTransmission",
                "PhononBoundaryDiffuse001", FatalException, msg);
  ...
  const G4int mode = GetPolarization(aStep.GetTrack());

  const G4VTouchable* nextVolTouchable = postPoint->GetTouchable();
  ...
  const G4ThreeVector incidentVDir = prePoint->GetMomentumDirection().unit();

  G4ThreeVector generalizedSurfNorm =
      G4CMP::GetSurfaceNormal(aStep, incidentVDir);

  generalizedSurfNorm = generalizedSurfNorm.unit();

  // generate diffuse transmitted wavevector

  const G4int maxTries = 10000;

  G4ThreeVector transmittedKGlobal;

  G4bool successfulTransmission = false;

  for (G4int iTry = 0; iTry < maxTries; ++iTry) {
    transmittedKGlobal = G4CMP::LambertReflection(-generalizedSurfNorm);

    transmittedKGlobal = transmittedKGlobal.unit();

    if (G4CMP::PhononVelocityIsOutward(latFar, mode, transmittedKGlobal,
                                       generalizedSurfNorm, nextVolTouchable,
                                       surfacePoint)) {
      successfulTransmission = true;
  ...
  if (!successfulTransmission) {
    G4String msg = G4PhononPolarization::Name(mode) +
                   " diffuse transmission failed after repeated "
                   "Lambertian sampling attempts.";

    G4Exception("G4CMPPhononBoundaryProcess::DoDiffuseTransmission",
                "PhononBoundaryDiffuse002", JustWarning, msg.c_str());
  ...
  anharmonicDecay->SetLattice(latFar);

  UpdateSCAfterLatticeChange();

  G4ThreeVector transmittedKLocal = transmittedKGlobal;

  G4CMP::RotateToLocalDirection(nextVolTouchable, transmittedKLocal);

  transmittedKLocal = transmittedKLocal.unit();

  G4ThreeVector transmittedVGlobal =
      latFar->MapKtoVDir(mode, transmittedKLocal);

  const G4double transmittedSpeed = latFar->MapKtoV(mode, transmittedKLocal);

  G4CMP::RotateToGlobalDirection(nextVolTouchable, transmittedVGlobal);

  transmittedVGlobal = transmittedVGlobal.unit();

  trackInfo->SetWaveVector(transmittedKGlobal);

  particleChange.ProposeVelocity(transmittedSpeed);

  particleChange.ProposeMomentumDirection(transmittedVGlobal);

  if (verboseLevel > 2) {
    G4cout << "\n"
           << GetProcessName() << "::DoDiffuseTransmission"
           << "\n  " << prePoint->GetPhysicalVolume()->GetName() << " -> "
           << postPoint->GetPhysicalVolume()->GetName()
           << "\n  mode = " << G4PhononPolarization::Label(mode)
           << "\n  surface normal = " << generalizedSurfNorm
           << "\n  k(global) = " << transmittedKGlobal
           << "\n  k(destination local) = " << transmittedKLocal
           << "\n  vg(global) = " << transmittedVGlobal
           << "\n  speed = " << transmittedSpeed / (m / s) << " m/s"
           << "\n  reflection count = " << trackInfo->ReflectionCount()
           << G4endl;
  }
}

// use the same wavevector as before for diffuse reflection
// but dont include specular

void G4CMPPhononBoundaryProcess::DoDiffuseReflection(
    const G4Track& aTrack, const G4Step& aStep,
    G4ParticleChange& /*particleChange*/) {
  if (verboseLevel > 5) {
    G4cout << "-- G4CMPPhononBoundaryProcess::DoDiffuseReflection --" << G4endl;
  }
  ...
  if (reflectedKDir.mag() < 1.e-10) {
    G4String msg = G4PhononPolarization::Name(mode) +
                   " diffuse reflection failed to generate a valid "
                   "Lambertian wavevector.";

    G4Exception((GetProcessName() + "::DoDiffuseReflection").c_str(),
                "BoundaryDiffuseReflection001", JustWarning, msg.c_str());
  ...
  if (!G4CMP::PhononVelocityIsInward(theLattice, mode, reflectedKDir, surfNorm,
                                     surfacePoint)) {
    G4String msg = G4PhononPolarization::Name(mode) +
                   " diffuse reflection generated an invalid outgoing "
                   "group-velocity direction.";

    G4Exception((GetProcessName() + "::DoDiffuseReflection").c_str(),
                "BoundaryDiffuseReflection002", JustWarning, msg.c_str());
  ...
  if (verboseLevel > 2) {
    ...
    G4cout << GetProcessName() << "::DoDiffuseReflection"
           << "\n  volume = "
           << aStep.GetPreStepPoint()->GetPhysicalVolume()->GetName()
           << "\n  mode = " << G4PhononPolarization::Name(mode)
           << "\n  surface normal = " << surfNorm
           << "\n  reflected k(global) = " << reflectedKDir
           << "\n  reflected vg(global) = " << vdir
           << "\n  reflection count = " << trackInfo->ReflectionCount()
           << G4endl;
  }
}