...
int main(int argc, char* argv[]) {
  ...
  G4double pdfDouble = G4CMP::MaxwellBoltzmannPDF(temp, Etemp*2.);
  G4double pdfFive = G4CMP::MaxwellBoltzmannPDF(temp, Etemp*5.);
  G4double pdfTen = G4CMP::MaxwellBoltzmannPDF(temp, Etemp*10.);

  G4cout << "T = " << temp/kelvin << " K,"
	 << " E = kT = " << Etemp/eV << " eV" << G4endl
  ...
  G4double sum = 0., dE = Etemp/20.;
  for (G4double Ei=0.; Ei<20.*Etemp; Ei+=dE) {
