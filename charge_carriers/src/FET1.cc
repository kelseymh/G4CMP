...
FET1::FET1()
{
    messenger = new FETMessenger1(this);
}
...
void FET1::ReadFETConstantsFile(G4String filename)
{
...
void FET1::ReadPhononConstantsFile(G4String filename)
{
...
/*G4double FET1::CalculateChargeSupressionFactor(const G4Run* run)
{
...
void FET1::BuildFETTemplate(vector<vector<vector<G4double> > >& FETTemplate)
{
    /*
    for(int i=0; i<channels; ++i)
    {
        //for(int j=0; j<preTrig/dt-1; ++j)
        //    FETTemplate[i][i][j] = 0;

        for(int j=preTrig/dt; j<timeBins; ++j)
        {
            FETTemplate[i][i][j] = exp(-((j-preTrig/dt+1)*dt)/decayTime)*volt;
        }
    }
    */
    ...
    templateFile.close();
    vector<G4double> temp(4096*4,0);
    for(int i=0; i< 4096; ++i)
        for(int j=0; j<4; ++j)
            temp[j+i*4] = FETTemplate[0][j][i];
        MatWriter("template.mat","testiiii",4,4096,temp,0);
}

void FET1::CalculateTrace()
{
    ...
    G4double chargeSupressionFactor = .0028;
    ...
    */
    ...
    std::fstream elecIn("electrons");

    G4double t;
    while(!elecIn.eof())
    {
        elecIn >> position[0] >> position[1] >> position[2] >> t; 
        t *= s;
        elecT.push_back(t);
        position[0] *= m;
        position[1] *= m;
        position[2] = position[2]*m - 1.27*cm;
    ...
    std::fstream holeIn("holes");

    while(!holeIn.eof())
    {
        holeIn >> position[0] >> position[1] >> position[2] >> t; 
        t *= s;
        holeT.push_back(t);
        position[0] *= m;
        position[1] *= m;
...
void FET1::ReadRamoInputFile(const G4String& filename, vector<vector<G4double> >& X, vector<G4double>& V)
{
    X.clear();
    V.clear();
...
void FET1::SaveResults(const G4String& matFile, const G4String& varName, G4int numRows, G4int numColumns,const vector<G4double>& colData, G4bool append)
{
    MatWriter(matFile, varName, numRows, numColumns, colData, append);
}

G4bool FET1::QuadrantFlip(const G4int &quadrant, vector< vector<G4double> > &elecPos, vector< vector<G4double> > &holePos)
{
