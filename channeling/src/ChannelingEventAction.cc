...
/// \file analysis/A01/src/ChannelingEventAction.cc
/// \brief Implementation of the ChannelingEventAction class
//
// $Id: 54d9fb408033dadd70f71c3e5759f493415c4bb4 $
// --------------------------------------------------------------
//
...
ChannelingEventAction::ChannelingEventAction()
{
    fSD_ID = -1;
    fSCI_ID = -1;
    fRBSD_ID = -1;
    ...
    fVerboseLevel = 0;
    fMessenger = new ChannelingEventMessenger(this);
}
...
void ChannelingEventAction::SetFileName(const G4String& vFileName){
    fFileOutSCI.close();
    fFileOutSD.close();
    fFileOutRBSD.close();
...
G4String ChannelingEventAction::GetFileName(){
    return fFileName;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void ChannelingEventAction::BeginOfEventAction(const G4Event*){
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void ChannelingEventAction::EndOfEventAction(const G4Event* evt)
{
    ...
    ChannelingDriftHitsCollection* fSD = 0;
    ChannelingDriftHitsCollection* fSCI = 0;
    ChannelingDriftHitsCollection* fRBSD = 0;
    
    G4HCofThisEvent * HCE = evt->GetHCofThisEvent();
    
    if(HCE)
    {
        //fSD = dynamic_cast<ChannelingDriftHitsCollection*>(aHC);

        if(fSCI_ID != -1){
            G4VHitsCollection* aHCSCI = HCE->GetHC(fSCI_ID);
            fSCI = (ChannelingDriftHitsCollection*)(aHCSCI);
        }
        
        if(fSD_ID != -1){
            G4VHitsCollection* aHCSD = HCE->GetHC(fSD_ID);
            fSD = (ChannelingDriftHitsCollection*)(aHCSD);
        }
        
        if(fRBSD_ID != -1){
            G4VHitsCollection* aHCRBSD = HCE->GetHC(fRBSD_ID);
            fRBSD = (ChannelingDriftHitsCollection*)(aHCRBSD);
        }
    }
    ...
    if(fSD && fFileOutSD)
    {
        ...
        for(int i2=0;i2<3;i2++){
            ...
            {
                ChannelingDriftHit* aHit = (*fSD)[i1];
    ...
    if(fSCI && fFileOutSD)
    {
        int n_hit_sd = fSCI->entries();
            for(int i1=0;i1<n_hit_sd;i1++)
            {
                ChannelingDriftHit* aHit = (*fSCI)[i1];
    ...
    if(fRBSD && fFileOutRBSD)
    {
        ...
        for(int i1=0;i1<n_hit_sd;i1++)
        {
            ChannelingDriftHit* aHit = (*fRBSD)[i1];
